...
export const ActivityBox : React.FC<{ activity: ActivityItem}> = ({activity}) => {
    ...
    return (
        <article
            className={`flex flex-col shadow-xl mx-auto max-w-sm bg-${activityBgColor} py-10 px-12 transform duration-500 hover:-translate-y-2 cursor-pointer max-h-190 rounded-md ${activityAdditionalClasses}`}>
            {
                activity.logo && (
                ...
                activity.link && (
                    <div className={"text-center mt-5"}>
                        <a href={activity.link} target={"_blank"}
                            className={`bg-white hover:bg-${activityBgColor} text-gray-800 hover:text-white hover:border hover:border-white font-bold py-2 px-4 rounded inline-flex items-center`} rel="noreferrer">
                            <svg xmlns="http://www.w3.org/2000/svg" className="ill-current w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                            </svg>
                            <span>
                                {t('section_activity_gotolink') as string}
                            </span>
                        </a>
                    </div>
                )
